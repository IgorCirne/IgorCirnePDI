...
int main(int argc, char** argv) {
  ...
  // Modificado para extrair contornos internos
  ...
  std::cout << "Total de contornos encontrados: " << contours.size()
            << std::endl;

  // Mostrar quantidade de pontos por contorno
  for (size_t i = 0; i < contours.size(); i++) {
    std::cout << "Contorno " << i << " tem " << contours[i].size() << " pontos."
              << std::endl;
  }
